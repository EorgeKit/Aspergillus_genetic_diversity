...
#PBS -N BLASTN_aflatoxin_cluster_AF13-ABGC-AY510451_to_announced_contigs
...
af_rep_seqs=/mnt/lustre/users/maloo/allan_project/allan-George/analysis/aflavus_rep_seqs_aligned
blastx_repseqs_out=/mnt/lustre/users/maloo/allan_project/allan-George/analysis/blastx


#THe following code block performs BLASTN of the aflatoxin gene cluster with genbank accession AY510451 to the announced contigs
for contig_db in $(ls $ann_contig_db)
do
    echo "This is announced contigs sample: $contig_db"
    echo
    echo "Running BLASTN of aflatoxin gene/cluster AY510451 on Contigs for sample $contig_db"
    echo

    mkdir -p $out_dir/$contig_db

    blastn \
    -query $af_gene_clusters/AF13-ABGC-AY510451.fasta \
    -db $ann_contig_db/$contig_db/Aspergillus_contig_$contig_db \
    -outfmt 7 \
    -num_threads 46 -mt_mode 0 -perc_identity 90 \
    > $out_dir/$contig_db/BlastN_AF13-ABGC-AY510451_2_$(basename $contig_db).tab
    
    echo
    echo "Finished running BLASTN of aflatoxin gene/cluster AF13-ABGC-AY510451 on Contigs for sample $contig_db"
    echo
done

##The following script performs TBLASTN of all obtained gene clsuter proteins to announced contigs
